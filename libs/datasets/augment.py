...
class normalize_rgb():
    def __init__(self,mean_R, mean_G, mean_B, MAX_value):
        self.mean_R = torch.tensor(mean_R)
        self.mean_G = torch.tensor(mean_G)
        self.mean_B = torch.tensor(mean_B)
        self.MAX_value = torch.tensor(MAX_value)
    def __call__(self,img):
        self.mean_R = torch.as_tensor(self.mean_R,dtype=torch.float32,device=img.device)
        self.mean_G = torch.as_tensor(self.mean_G,dtype=torch.float32,device=img.device)
        self.mean_B = torch.as_tensor(self.mean_B,dtype=torch.float32,device=img.device)
        self.MAX_valueean_B = torch.as_tensor(self.MAX_value,dtype=torch.float32,device=img.device)
        img[0,:,:] = (img[0,:,:]-self.mean_R)/self.MAX_value
        img[1,:,:] = (img[1,:,:]-self.mean_G)/self.MAX_value
        img[2,:,:] = (img[2,:,:]-self.mean_B)/self.MAX_value
        return img
