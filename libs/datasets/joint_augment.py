...
class TriCompose():
    ...
    def __call__(self, img, mask, scale):
        # assert img.size == mask.size
        for t in self.transforms:
            img, mask, scale = t(img, mask, scale)
...
class To_Tensor():
    def __call__(self, arr, arr2):
        ...
        arr = torch.from_numpy(np.array(arr).astype(np.float32).transpose(2,0,1))
        arr2 = torch.from_numpy(np.array(arr2).astype(np.float32).transpose(2,0,1))
        return arr, arr2

class To_Tensor_Camvid():
    def __call__(self, arr, arr2):
        if len(np.array(arr).shape) == 2:
            arr = np.array(arr)[:,:,None]
        if len(np.array(arr2).shape) == 2:
...
class Tri_To_PIL_Image():
    def __call__(self, img, mask, scale):
        img = to_pil_image(img)
        mask = to_pil_image(mask)
        scale=to_pil_image(scale)
        return img, mask, scale
...
class TriRandomRotate():
    ...
    def __call__(self, img, mask, scale):
        if random.random() < self.prob:
            rotate_detree = random.uniform(self.degrees[0], self.degrees[1])
            return img.rotate(rotate_detree, Image.BILINEAR), mask.rotate(rotate_detree, Image.NEAREST), scale.rotate(rotate_detree, Image.BILINEAR)
        return img, mask, scale
...
class TriRandomAffine(object):
    ...
    def __call__(self, img, mask, scale_):
        """
            img (PIL Image): Image to be transformed.
        ...
        if random.random() < self.prob:
            ret = self.get_params(self.degrees, self.translate, self.scale, self.shear, img.size)
            return F.affine(img, *ret, resample=self.resample, fillcolor=self.fillcolor), F.affine(mask, *ret, resample=self.resample, fillcolor=self.fillcolor), F.affine(scale_, *ret, resample=self.resample, fillcolor=1)
        return img, mask, scale_
