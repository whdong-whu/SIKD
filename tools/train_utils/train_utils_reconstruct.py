...
def my_load_checkpoint(model=None, optimizer=None, filename="checkpoint", logger=None):
    if os.path.isfile(filename):
        ...
        if model is not None and checkpoint['model_state'] is not None:
            pretrained_param = {k: v for k, v in checkpoint['model_state'].items()
                                if k not in ['Conv_1x1.weight', 'Conv_1x1.bias']}
            param.update(pretrained_param)
...
class Trainer():
    ...
    def _train_it(self, batch, epoch=0):
        ...
        loss, tb_dict, disp_dict = self.model_fn(self.model, batch, self.criterion, perfermance=False, epoch=0, num_class=self.cfg.DATASET.NUM_CLASS, imgmean=self.imgmean, kd=self.kd)
    ...
    def train(self, start_it, start_epoch, n_epochs, train_loader, test_loader=None,
              ckpt_save_interval=5, lr_scheduler_each_iter=False, best_res=1.0):
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            # save param of every training epoch
            # ckpt_name = os.path.join(self.ckpt_dir, "checkpoint_epoch_%d" % trained_epoch)
            # save_checkpoint(checkpoint_state(self.model, self.optimizer, trained_epoch, it, kd=self.kd),
            #                 filename=ckpt_name)
