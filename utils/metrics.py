...
def dice(pred, target):
    ...
    dice = (2 * intersection) / (pred_flat.sum().item() + target_flat.sum().item() + smooth)
    return dice

def dice3D_Brats(img_gt, img_pred, voxel_size):
    """
...
def hd_3D95(img_pred, img_gt, labels=[3, 1, 2]):
    res = []
    for c in labels:
        gt_c_i = np.copy(img_gt)
        gt_c_i[gt_c_i != c] = 0
        ...
        if np.sum(pred_c_i) == 0 or np.sum(gt_c_i) == 0:
            ...
            hausdorff = hd95(pred_c_i, gt_c_i)
